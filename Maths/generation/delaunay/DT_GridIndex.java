...
public class DT_GridIndex {
   ...
   */
  public DT_GridIndex(DelaunayTriangulation delaunay, int xCellCount,
      int yCellCount) {
  ...
  /**
   * Constructs a grid index holding the triangles of a delaunay triangulation.
   * The grid will be made of (xCellCount * yCellCount) cells. The smaller the
   * cells the less triangles that fall in them, whuch means better indexing,
   * but also more cells in the index, which mean more storage. The smaller the
   * indexed region is, the smaller the cells can be and still maintain the same
   * capacity, but adding geometries outside the initial region will invalidate
   * the index !
   * 
   * @param delaunay
   *          delaunay triangulation to index
   ...
   */
  public DT_GridIndex(DelaunayTriangulation delaunay, int xCellCount,
      int yCellCount, DT_BoundingBox region) {
   ...
   *          geographic region to index
   */
  private void init(DelaunayTriangulation delaunay, int xCellCount,
      int yCellCount, DT_BoundingBox region) {
    ...
    grid = new DT_Triangle[xCellCount][yCellCount];

    DT_Triangle colStartTriangle = indexDelaunay.find(middleOfCell(0, 0));
   ...
   */
  public DT_Triangle findCellTriangleOf(DT_Point point) {
   ...
   *          triangles, removed triangles or both. All that matter is that they
   *          cover the changed area.
   */
  public void updateIndex(Iterator<DT_Triangle> updatedTriangles) {

    // Gather the bounding box of the updated area
    DT_BoundingBox updatedRegion = new DT_BoundingBox();

    while (updatedTriangles.hasNext()) {
      updatedRegion = updatedRegion.unionWith(updatedTriangles.next()
          .getBoundingBox());
    }
    ...
    if (!indexRegion.contains(updatedRegion)) {
      init(indexDelaunay, (int) (indexRegion.getWidth() / x_size),
      ...
      DT_Triangle adjacentValidTriangle = findValidTriangle(minInvalidCell);
      updateCellValues(minInvalidCell.x, minInvalidCell.y, maxInvalidCell.x,
          maxInvalidCell.y, adjacentValidTriangle);
    }
  }

  private void updateCellValues(int startXCell, int startYCell, int lastXCell,
      int lastYCell, DT_Triangle startTriangle) {
    ...
    for (int i = startXCell; i <= lastXCell; i++) {
      ...
      startTriangle = indexDelaunay.find(middleOfCell(i, startYCell),
          startTriangle);
      grid[i][startYCell] = startTriangle;
      DT_Triangle prevRowTriangle = startTriangle;
   ...
   */
  private DT_Triangle findValidTriangle(Point minInvalidCell) {
    ...
    if (minInvalidCell.x == 0 && minInvalidCell.y == 0)
      return indexDelaunay.find(
          middleOfCell(minInvalidCell.x, minInvalidCell.y), null);
    else
   ...
   */
  private Point getCellOf(DT_Point coordinate) {
