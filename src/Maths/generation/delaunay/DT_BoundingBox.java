...
public class DT_BoundingBox {
    ...
    public DT_BoundingBox(DT_BoundingBox other) {
        if (other.isNull())
            ...
            init(other.minx, other.maxx, other.miny, other.maxy, other.minz,
                    other.maxz);
    }
     ...
     */
    public DT_BoundingBox(double minx, double maxx, double miny, double maxy,
     ...
     */
    public DT_BoundingBox(DT_Point lowerLeft, DT_Point upperRight) {
        init(lowerLeft.x, upperRight.x, lowerLeft.y, upperRight.y, lowerLeft.z,
                upperRight.z);
    }
     ...
     *          the second y-value
     */
    private void init(double x1, double x2, double y1, double y2, double z1,
            double z2) {
        if (x1 < x2) {
            minx = x1;
            maxx = x2;
     ...
     *
     * @return true if this BoundingBox is uninitialized or is the envelope of the
     *         empty geometry.
     */
    ...
    public boolean contains(DT_BoundingBox other) {
        return !(isNull() || other.isNull()) && other.minx >= minx
                && other.maxy <= maxx && other.miny >= miny && other.maxy <= maxy;
    }
    ...
    public DT_BoundingBox unionWith(DT_BoundingBox other) {
        ...
        if (isNull()) {
            ...
            return new DT_BoundingBox(Math.min(minx, other.minx), Math.max(maxx,
                    other.maxx), Math.min(miny, other.miny), Math.max(maxy, other.maxy),
     ...
     */
    public DT_Point getMinPoint() {
        return new DT_Point(minx, miny, minz);
    }
    
    /**
     * @return Minimum coordinate of bounding box
     */
    public DT_Point getMaxPoint() {
        return new DT_Point(maxx, maxy, maxz);
    }
}