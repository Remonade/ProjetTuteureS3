...
public class DT_Triangle {
    DT_Point a, b, c;
    DT_Triangle abnext, bcnext, canext;
    DT_Circle circum;
    ...
    public DT_Triangle(DT_Point A, DT_Point B, DT_Point C) {
        // visitflag=visitValue;
        a = A;
        int res = C.pointLineTest(A, B);
        if ((res <= DT_Point.LEFT) || (res == DT_Point.INFRONTOFA)
                || (res == DT_Point.BEHINDB)) {
            b = B;
            c = C;
        } else { // RIGHT
     ...
     * @param B
     */
    public DT_Triangle(DT_Point A, DT_Point B) {
        // visitflag=visitValue;
        a = A;
        b = B;
        halfplane = true;
    ...
    */
    ...
    public DT_BoundingBox getBoundingBox() {
        DT_Point lowerLeft, upperRight;
        lowerLeft = new DT_Point(Math.min(a.x(), Math.min(b.x(), c.x())), Math.min(
                a.y(), Math.min(b.y(), c.y())));
        upperRight = new DT_Point(Math.max(a.x(), Math.max(b.x(), c.x())),
                Math.max(a.y(), Math.max(b.y(), c.y())));
        return new DT_BoundingBox(lowerLeft, upperRight);
    }
    
    void switchneighbors(DT_Triangle Old, DT_Triangle New) {
        if (abnext == Old)
            abnext = New;
        else if (bcnext == Old)
            bcnext = New;
    ...
    DT_Triangle neighbor(DT_Point p) {
     ...
     */
    DT_Triangle nextNeighbor(DT_Point p, DT_Triangle prevTriangle) {
    ...
    DT_Circle circumcircle() {
        ...
        if (den == 0) // oops, degenerate case
            circum = new DT_Circle(a, Double.POSITIVE_INFINITY);
        else {
            DT_Point cen = new DT_Point((u * (b.y - c.y) - v * (a.y - b.y)) / den, (v
                    * (a.x - b.x) - u * (b.x - c.x))
                    / den);
            circum = new DT_Circle(cen, cen.distance2(a));
        }
        return circum;
    }
    
    boolean circumcircle_contains(DT_Point p) {
    ...
    public boolean contains(DT_Point p) {
        ...
        if ((a12 == DT_Point.LEFT && a23 == DT_Point.LEFT && a31 == DT_Point.LEFT)
                || (a12 == DT_Point.RIGHT && a23 == DT_Point.RIGHT && a31 == DT_Point.RIGHT)
                || (a12 == DT_Point.ONSEGMENT || a23 == DT_Point.ONSEGMENT || a31 == DT_Point.ONSEGMENT))
            ans = true;
     ...
     */
    public boolean contains_BoundaryIsOutside(DT_Point p) {
        boolean ans = false;
        if (this.halfplane | p == null) return false;
     ...
     *         By Eyal Roth & Doron Ganel.
     */
    public boolean isCorner(DT_Point p) {
        return (p.x == a.x & p.y == a.y) | (p.x == b.x & p.y == b.y)
                | (p.x == c.x & p.y == c.y);
    }
    
    // Doron
    public boolean fallInsideCircumcircle(DT_Point[] arrayPoints) {
     ...
     */
    public double z_value(DT_Point q) {
    ...
    /**
     * compute the Z value for the X,Y values of q. assume this triangle represent
     * a plane --> q does NOT need to be contained in this triangle.
     *
     * @param x
    ...
    public double z(double x, double y) {
        return z_value(new DT_Point(x, y));
    }
