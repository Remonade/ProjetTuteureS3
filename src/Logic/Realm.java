...
public class Realm {
	...
	private Realm(String name, int id) {
		m_name = name;
		m_id = id;
		System.out.println("\n\n\n\t\tNEW REALM(" + id + ") " + name);
	}
	...
	public void addEntity(ArrayList<Entity> liste) {
		for (Entity e : liste) {
			addEntity(e);
		}
	}
	...
	public void addEntity(Entity e) {
		if (e != null) {
			if (e instanceof EntityParticle) {
				...
				System.out.println("new Waypoint added to realm " + m_id + " with ID " + m_wayPoints.size());
				((EntityWayPoint) e).setID(m_wayPoints.size());
				((EntityWayPoint) e).setRealm(m_id);
